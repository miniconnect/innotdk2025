...
public class ReplMain {
    ...
    public void run(String configFilePath) {
        try (Engine engine = loadDynamicEngine(configFilePath)) {
    ...
    private MiniSession createCompundSession(Engine engine) {
        FrameworkSession frameworkSession = new FrameworkSessionManager(engine).openSession();
        // TODO: add calcite session too
        return new FallbackChainMiniSession(frameworkSession);
    }
